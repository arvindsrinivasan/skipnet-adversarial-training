...
def parse_args():
    ...
    parser.add_argument('--dataset', '-d', default='cifar10', type=str,
                        choices=['cifar10', 'cifar100', 'adversarial'],
                        help='dataset type')
    ...
    parser.add_argument('--train-alphas', default="0.0,0.25,0.5,0.75,1.0", type=str)
    parser.add_argument('--test-alphas', default="0.0,0.25,0.5,0.75,1.0", type=str)
    parser.add_argument('--size', default=1200, type=int)
    parser.add_argument('--acc-adv', action="store_true")
...
def run_training(args):
    ...
    train_loader = prepare_train_data(dataset=args.dataset,
                                      ...
                                      size=args.size,
                                      alphas=args.train_alphas,
                                      acc_adv=args.acc_adv,
    ...
    test_loader = prepare_test_data(dataset=args.dataset,
                                    ...
                                    adversarial=True,
                                      size=args.size,
                                      alphas=args.test_alphas,
                                      acc_adv=args.acc_adv,
                                      image_folder=args.image_dir)
    if('adversarial' in args.dataset):
        test_loader_og = prepare_test_data(dataset=args.dataset,
                                        batch_size=args.batch_size,
                                        shuffle=False,
                                        num_workers=args.workers,
                                        adversarial=False,
    ...
    #print("Start Iter: {}\nNum Iters: {}".format(args.start_iter, args.iters))
    args.start_iter = 0
    ...
    for i in range(args.start_iter, args.iters):
        ...
        if (i % args.eval_every == 0) or (i == (args.iters-1)):     
            print("x-------x-------x")
            print("Adversarial Test Set Performance")
            prec1 = validate(args, test_loader, model, criterion)
            print("-----------------")
            print("Original Test Set Performance")
            prec1 = validate(args, test_loader_og, model, criterion)
            print("x-------x-------x")
...
def test_model(args):
    ...
    print("Original Test Set Performance")
    validate(args, test_loader_og, model, criterion)
